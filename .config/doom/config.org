...
#+END_SRC
** Title
Include project and file/buffer name in the title
#+begin_src emacs-lisp
...
#+end_src
*** Quickshell
For Quickshell, create an empty file named .qmlls.ini in the same directory as your shell.qml. Quickshell will replace it with a managed qmlls configuration, and the docs recommend gitignoring it because it is machine-specific:
https://quickshell.org/docs/master/guide/install-setup
https://quickshell.org/docs/v0.2.1/guide/install-setup/#language-server

So if your shell entrypoint is, for example, ~/.config/quickshell/shell.qml, then do:

touch ~/.config/quickshell/.qmlls.ini

A few caveats from the docs:

- qmlls works poorly when the file structure is temporarily broken.
- Quickshell type docs are not available through the LSP.
- PanelWindow may not resolve correctly.
** Odin
#+begin_src emacs-lisp
